...
gst-launch-1.0 udpsrc port=5000 ! application/x-rtp,encoding-name=H263-1998,payload=96,a-framerate=10 ! rtph263pdepay ! h263parse ! avdec_h263 ! autovideosink
\n************\n"
read -p "Press anykey to start streaming at select resolution "$width"x"$height anykey

gst-launch-1.0 -v v4l2src ! 'video/x-raw,width='$width',height='$height',format=UYVY,framerate=10/1' ! videoconvert ! avenc_h263p ! h263parse ! rtph263ppay ! udpsink host=$ip_addr port=5000


#gst-launch-1.0 -v v4l2src ! 'video/x-raw,width=640,height=480,format=UYVY,framerate=15/1' ! videoconvert ! avenc_h263p ! h263parse ! rtph263ppay ! udpsink host=192.168.0.129 port=5000
